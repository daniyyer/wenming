QUnit.test("Testing silentMoveTo vertical with anchor", function (assert) {
  ...
  assert.deepEqual(
    getTransformFromElement($(id)),
    ["0", `${-(windowHeight * 2)}`, "0"],
    `We expect sections transformation to be [0, ${-(windowHeight * 2)}, 0]`,
  );
  assert.ok(
    FP.test.options.scrollingSpeed,
    1000,
    "We expect scrollingSpeed to be the samea s the initial one after the silent move",
  );
});
...
QUnit.test("Testing silentMoveTo verticall + horizontal with anchors", function (assert) {
  ...
  assert.equal(
    ...
    2,
    "We expect slide 2.3 to be active",
  );
  var slidesTranforms = getTransformFromElement(
    $(id).find(SECTION_ACTIVE_SEL).find(SLIDES_CONTAINER_SEL),
  );
  assert.deepEqual(
    slidesTranforms,
    ["-2000", "0", "0"],
    "We expect slides tranformation to be [-2000, 0, 0]",
  );
...
QUnit.test("Testing silentMoveTo horizontal only with anchor", function (assert) {
  ...
  assert.equal(
    $(SECTION_ACTIVE_SEL).find(SLIDE_ACTIVE_SEL).index(),
    2,
    "We expect slide 1.3 to be active",
  );
  var slidesTranforms = getTransformFromElement(
    $(id).find(SECTION_ACTIVE_SEL).find(SLIDES_CONTAINER_SEL),
  );
