QUnit.test('.destroy("all") removes all inline CSS and fullpage classes', function (assert) {
  ...
  assert.equal(
    $SECTION_SEL.get(0).style.paddingTop,
    "",
    SECTION_SEL + " should have an empty string paddingTop",
  );
  assert.equal(
    $SECTION_SEL.get(0).style.paddingBottom,
    "",
    SECTION_SEL + " should have an empty string paddingBottom",
  );
  ...
  assert.equal(
    $html.hasClass(RESPONSIVE),
    false,
    "html should not have the " + RESPONSIVE + " class",
  );

  //destroyed
  assert.equal(
    $(id).hasClass(DESTROYED),
    true,
    "fullpage.js container should have the " + DESTROYED + " class",
  );

  //no table classes
  assert.equal(
    $(id).find(TABLE_CELL_SEL).length,
    0,
    "Sections or slides should not containt the " + TABLE_CELL_SEL + " class",
  );

  //no SLIDES_CONTAINER_SEL classes
  assert.equal(
    $(id).find(SLIDES_CONTAINER_SEL).length,
    0,
    "Sections or slides should not containt the " + SLIDES_CONTAINER_SEL + " class",
  );

  //no SLIDES_WRAPPER_SEL classes
  assert.equal(
    $(id).find(SLIDES_WRAPPER_SEL).length,
    0,
    "Sections or slides should not containt the " + SLIDES_WRAPPER_SEL + " class",
  );

  //maintains original styles
  assert.equal(
    $(id).find("#section2")[0].style.backgroundColor,
    "blue",
    "Sections or slides should maintain previous inline styles",
  );
  ...
  assert.equal(
    $(SECTION_SEL + ", " + SLIDE_SEL).find(SCROLLABLE_SEL).length,
    0,
    "Sections or slides should not containt the " + SCROLLABLE_SEL + " class",
  );
  assert.equal(
    $(SECTION_SEL + ", " + SLIDE_SEL).find(".fp-scroller").length,
    0,
    "Sections or slides should not containt the fp-scroller class",
  );

  //arrows
  var numArrowsAndBullets = $(document).find(
    SECTION_NAV_SEL + ", " + SLIDES_NAV_SEL + ", " + SLIDES_ARROW_SEL,
  ).length;
  assert.equal(
    numArrowsAndBullets,
    0,
    "html should not contain any of the bullets or arrows generated by fullPage.js",
  );
  ...
  assert.deepEqual(
    ...
    ["0", "0", "0"],
    "fullPage.js container should have no transformations",
  );
  ...
  $.each(usedSelectors, function (index, value) {
    assert.equal(
      $(id).find(value).length,
      0,
      "fullpage.js container should not contain any internal selectors generated by fullPage.js",
    );
  });
});
/*
...
QUnit.test('.destroy("all") with css3:true & autoScrolling:false', function (assert) {
  ...
  var FP = initFullpageNew(
    id,
    Object.assign({}, allBasicOptions, { css3: true, autoScrolling: false }),
  );
...
QUnit.test('.destroy("all") with css3:true & autoScrolling:false', function (assert) {
  checkDestroyWhenScrollBarPresent(
    assert,
    Object.assign({}, allBasicOptions, { css3: true, autoScrolling: false }),
  );
});
...
QUnit.test('.destroy("all") with css3:true & scrollBar:true', function (assert) {
  checkDestroyWhenScrollBarPresent(
    assert,
    Object.assign({}, allBasicOptions, { css3: true, scrollBar: true }),
  );
});
...
QUnit.test('.destroy("all") with css3:false & autoScrolling:false', function (assert) {
  checkDestroyWhenScrollBarPresent(
    assert,
    Object.assign({}, allBasicOptions, { css3: false, autoScrolling: false }),
  );
});
...
QUnit.test('.destroy("all") with css3:false & scrollBar:true', function (assert) {
  checkDestroyWhenScrollBarPresent(
    assert,
    Object.assign({}, allBasicOptions, { css3: false, scrollBar: true }),
  );
});
