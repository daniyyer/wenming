...
QUnit.test("Testing setAllowScrolling(false) with moveSectionUp & moveSectionDown", function (
  assert,
) {
  var id = "#fullpage";
  var FP = initFullpageNew(id, Object.assign({}, allBasicOptions, { loopTop: true }));
...
QUnit.test("Testing setAllowScrolling(false) with keydown down & up", function (assert) {
  ...
  var FP = initFullpageNew(
    id,
    Object.assign({}, allBasicOptions, { scrollingSpeed: 600, loopTop: true }),
  );
  var windowHeight = $(window).height();
...
QUnit.test('Testing setAllowScrolling(false, "up, left")', function (assert) {
  ...
  $(SECTION_ACTIVE_SEL)
    .find(SLIDES_ARROW_SEL + SLIDES_PREV_SEL)[0]
    .click();
