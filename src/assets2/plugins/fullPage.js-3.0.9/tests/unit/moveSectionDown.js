QUnit.test(
  "Testing moveSectionDown first section active with slides {css3:true, autoScrolling:true}",
  function (assert) {
    var id = "#fullpage";
    ...
    assert.deepEqual(
      getTransform(FP.test.translate3d),
      ["0", `${-(windowHeight * 2)}`, "0"],
      `We expect sections transformation to be [0, ${-(windowHeight * 2)}, 0]`,
    );
...
QUnit.test(
  "Testing moveSectionDown first section active with slides {css3:false, autoScrolling:true}",
  function (assert) {
    var id = "#fullpage";
    ...
    assert.deepEqual(
      FP.test.top,
      -(windowHeight * 1) + "px",
      `We expect wrapper top property to be -${windowHeight * 1}px`,
    );
    assert.equal($(id).find(SECTION_ACTIVE_SEL).index(), 1, "We expect section 2 to be active");
    FP.moveSectionDown();
    ...
    assert.deepEqual(
      FP.test.top,
      -(windowHeight * 2) + "px",
      `We expect wrapper top property to be -${windowHeight * 2}px`,
    );
    assert.equal($(id).find(SECTION_ACTIVE_SEL).index(), 2, "We expect section 3 to be active");
  },
);
...
QUnit.test(
  "Testing moveSectionDown in fullpage-last-section-active {css3:true, autoScrolling:true}",
  function (assert) {
    ...
    assert.deepEqual(
      getTransform(FP.test.translate3d),
      ["0", `${-(windowHeight * 3)}`, "0"],
      `We expect sections transformation to be [0, ${-(windowHeight * 3)}, 0]`,
    );
...
QUnit.test(
  "Testing moveSectionDown in fullpage-last-section-active {css3:false, autoScrolling:true}",
  function (assert) {
    var id = "#fullpage-last-section-active";
    ...
    assert.deepEqual(
      FP.test.top,
      -(windowHeight * 3) + "px",
      `We expect wrapper top property to be -${windowHeight * 3}px`,
    );
