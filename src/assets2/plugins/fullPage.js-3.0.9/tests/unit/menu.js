...
QUnit.test("Testing menu. Moving it outside the library wrapper when css3:true", function (assert) {
  ...
  $(id).append($("#menu"));
  assert.equal(
    $(id).find("#menu").closest(id).length,
    1,
    "We expect menu to be inside the fullpage wrapper",
  );
  ...
  assert.equal(
    $(id).find("#menu").closest(id).length,
    0,
    "We expect menu to be outside the fullpage wrapper",
  );
});

QUnit.test("Testing menu. Keeping it inside the library wrapper when css3:false", function (
  assert,
) {
  var id = "#fullpage";
...
QUnit.test("Testing menu `active` class when auto scrolling", function (assert) {
  ...
  // first menu
  assert.equal(
    $("#menu").find(".active[data-menuanchor]").index(),
    0,
    "We expect item 1 to be active",
  );
  assert.equal(
    $("#menu").find(".active[data-menuanchor]").length,
    1,
  ...
  // second menu
  assert.equal(
    $("#menu-two").find(".active[data-menuanchor]").index(),
    0,
    "We expect item 1 to be active",
  );
  assert.equal(
    $("#menu-two").find(".active[data-menuanchor]").length,
    1,
    "We expect a single item to be active",
  );
...
QUnit.test("Testing menu `active` class when auto scrolling and sliding horizontally", function (
  assert,
) {
...
QUnit.test("Testing menu `active` class when scrolling & autoScrolling:false", function (assert) {
  ...
  var FP = initFullpageNew(
    id,
    Object.assign({}, allBasicOptions, { autoScrolling: false, menu: ".menu" }),
  );
