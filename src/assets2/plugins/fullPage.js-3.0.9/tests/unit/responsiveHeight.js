...
function checkIsNotResponsive(FP, assert) {
  assert.ok(
    !$("body").hasClass(RESPONSIVE),
    "We expect RESPONSIVE class to not be applied on the body",
  );
  ...
  assert.equal(
    afterResponsive,
    false,
    "We expect afterResponsive callback not to have been fired yet or to have value false",
  );
}
