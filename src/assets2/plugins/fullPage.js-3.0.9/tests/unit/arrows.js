...
QUnit.test("Testing controlArrows:true & loopHorizontal:false in 1st slide", function (assert) {
  ...
  assert.equal(
    $(SLIDES_ARROW_SEL + SLIDES_NEXT_SEL + ":visible").length,
    1,
    "We expect one next control arrow to exist",
  );
  assert.equal(
    $(SLIDES_ARROW_SEL + SLIDES_PREV_SEL + ":visible").length,
    0,
    "We expect 0 prev control arrow to exist",
  );
});
...
QUnit.test("Testing controlArrows:true & loopHorizontal:false in last slide", function (assert) {
  ...
  assert.equal(
    $(SLIDES_ARROW_SEL + SLIDES_PREV_SEL + ":visible").length,
    1,
    "We expect 1 prev control arrow to exist",
  );
  assert.equal(
    $(SLIDES_ARROW_SEL + SLIDES_NEXT_SEL + ":visible").length,
    0,
    "We expect 0 next control arrow to exist",
  );
});
...
QUnit.test("Testing controlArrowColor", function (assert) {
  ...
  assert.equal(
    $(SLIDES_ARROW_SEL + ":visible").first()[0].style.borderColor,
    "transparent blue transparent transparent",
    "We expect the prev arrow to be blue",
  );
  assert.equal(
    $(SLIDES_ARROW_SEL + ":visible").last()[0].style.borderColor,
    "transparent transparent transparent blue",
    "We expect the next arrow to be blue",
  );
});
