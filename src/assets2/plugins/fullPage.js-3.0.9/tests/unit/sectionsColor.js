...
QUnit.test("Testing sectionsColor default", function (assert) {
  ...
  for (var i = 0; i < $(SECTION_SEL).length; i++) {
    assert.equal(
      $(SECTION_SEL)[i].style.backgroundColor,
      "",
      "We expect sections to not have any backgroundColor",
    );
  }

  for (var i = 0; i < $(SLIDE_SEL).length; i++) {
    assert.equal(
      $(SLIDE_SEL)[i].style.backgroundColor,
      "",
...
QUnit.test("Testing sectionsColor with colors", function (assert) {
  ...
  for (var i = 0; i < $(SECTION_SEL).length; i++) {
    assert.equal(
      rgb2hex($(SECTION_SEL)[i].style.backgroundColor),
      colors[i],
      "We expect sections to not have any backgroundColor",
    );
  }
