...
QUnit.test(
  "Testing continuousVertical:true with css3:false Checking middle slide stills in the middle",
  function (assert) {
    ...
    assert.equal(
      $(id).find(SECTION_ACTIVE_SEL).find(SLIDE_ACTIVE_SEL).index(),
      1,
      "We expect slide 1.2 to be active",
    );
    ...
    setTimeout(function () {
      ...
      assert.equal(
        $(id).find(SECTION_SEL).eq(0).find(SLIDE_ACTIVE_SEL).index(),
        1,
        "We expect slide 1.2 to be active",
      );
