QUnit.test("Testing sectionSelector default", function (assert) {
  ...
  assert.equal(
    ...
    $(id).find(".section").length,
    "We expect sectonSelector to be .section by default",
  );
});
...
QUnit.test("Testing slidesSelector default", function (assert) {
  ...
  assert.equal(
    $(id).find(SLIDE_SEL).length,
    $(id).find(".slide").length,
    "We expect slideSelector to be .slide by default",
  );
});
...
QUnit.test("Testing a custom sectionSelector ", function (assert) {
  ...
  assert.equal(
    $(id).find(SECTION_SEL).length,
    $(id).find(customSelector).length,
    "We expect sections to use a custom selector",
  );
});
...
QUnit.test("Testing a custom slideSelector", function (assert) {
  ...
  assert.equal(
    $(id).find(SLIDE_SEL).length,
    $(id).find(customSelector).length,
    "We expect slides to use a custom selector",
  );
});
