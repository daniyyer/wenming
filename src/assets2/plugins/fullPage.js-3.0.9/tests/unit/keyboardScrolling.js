...
function upAndDown(assert, done, FP, params, prefix) {
  ...
  setTimeout(function () {
    assert.deepEqual(
      ...
      ["0", `${-(windowHeight * 1)}`, "0"],
      prefix + `: We expect sections transformation to be [0, ${-(windowHeight * 1)}, 0]`,
    );
  ...
  setTimeout(function () {
    assert.deepEqual(
      getTransform(FP.test.translate3d),
      ["0", "0", "0"],
      prefix + `: We expect sections transformation to be [0, 0, 0]`,
    );
...
QUnit.test("Testing keyboardScrolling:true keys (tab, space, pageup, pagedown...)", function (
  assert,
) {
  var id = "#fullpage";
  var FP = initFullpageNew(
...
QUnit.test("Testing keyboardScrolling:false with moveSectionUp & moveSectionDown", function (
  assert,
) {
  var id = "#fullpage";
  var FP = initFullpageNew(
...
QUnit.test('Testing setKeyboardScrolling(false, "up")', function (assert) {
  ...
  var FP = initFullpageNew(
    id,
    Object.assign({}, allBasicOptions, { loopTop: true, scrollingSpeed: 50 }),
  );
  var windowHeight = $(window).height();
...
QUnit.test('Testing setKeyboardScrolling(false, "up, left")', function (assert) {
  ...
  setTimeout(function () {
    ...
    assert.deepEqual(
      transforms,
      [`-1000`, "0", "0"],
      `We expect slides transformation to be [-1000, 0, 0]`,
    );
    assert.equal(
      $(SECTION_ACTIVE_SEL).find(SLIDE_ACTIVE_SEL).index(),
      1,
      "We expect slide 2.2 to be active",
    );
    done();
