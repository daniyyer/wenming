...
QUnit.test(
  "Testing warnings for scrollOverflow:true with scrollBar:true & autoScrolling:true",
  function (assert) {
    var id = "#fullpage";
    warnFired = false;
    mockConsole();
    ...
    var FP = initFullpageNew(id, {
      scrollBar: true,
      scrollOverflow: true,
      scrollOverflowHandler: { init: function () {}, remove: function () {} },
    });
    assert.equal(isWarnFired(), true, "We expect console.warn to be fired");
  },
);
...
QUnit.test(
  "Testing warnings for scrollOverflow:true with scrollBar:false & autoScrolling:true",
  function (assert) {
    var id = "#fullpage";
    warnFired = false;
    mockConsole();
...
QUnit.test("Testing warnings for autoScrolling:true with continuousVertical:true", function (
  assert,
) {
  var id = "#fullpage";
  warnFired = false;
...
QUnit.test("Testing warnings for autoScrolling:false with continuousVertical:true", function (
  assert,
) {
  var id = "#fullpage";
  warnFired = false;
...
QUnit.test("Testing warnings for name element same as anchor?", function (assert) {
  ...
  var FP = initFullpageNew(id, {
    anchors: ["anchor-as-name", "test2", "test3", "test4"],
    licenseKey: licenseKey,
  });
  assert.equal(isErrorFired(), true, "We expect console.warn to be fired");
});
...
QUnit.test("Testing warnings for id element same as anchor?", function (assert) {
  ...
  var FP = initFullpageNew(id, {
    anchors: ["anchor-as-id", "test2", "test3", "test4"],
    licenseKey: licenseKey,
  });
  assert.equal(isErrorFired(), true, "We expect console.warn to be fired");
});
