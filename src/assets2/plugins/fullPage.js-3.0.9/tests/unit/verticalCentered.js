QUnit.test("Testing verticalCentered:true", function (assert) {
  ...
  assert.equal(
    $(TABLE_CELL_SEL).length,
    7,
    "We expect TABLE_CELL_SEL to exist for every section/slide",
  );
  assert.equal(
    $(TABLE_CELL_SEL).eq(0).css("height"),
    windowHeight + "px",
    "We expect TABLE_CELL_SEL to have the windows height",
  );

  assert.equal(
    $(TABLE_CELL_SEL).eq(0).closest(SECTION_SEL).length,
    1,
    "We expect TABLE_CELL_SEL to exist as a child of section",
  );
  assert.equal(
    $(TABLE_CELL_SEL).eq(1).closest(SLIDE_SEL).length,
    1,
    "We expect TABLE_CELL_SEL to exist as a child of slide within a slider",
  );
});
