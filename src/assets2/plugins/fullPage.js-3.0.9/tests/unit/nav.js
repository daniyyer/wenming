QUnit.test("Testing navigation exist when using navigation:true", function (assert) {
  ...
  assert.equal(
    $(SECTION_NAV_SEL).closest(id).length,
    0,
    "We expect navigation to be outside the fullpage wrapper",
  );
  assert.equal(
    $(SECTION_NAV_SEL).find("li").length,
    $(id).find(SECTION_SEL).length,
    "We expect navigation have as many items as sections exist",
  );
  assert.equal(
    $(SECTION_NAV_SEL).find("a").length,
    $(id).find(SECTION_SEL).length,
    "We expect navigation have as many links as sections exist",
  );
  assert.equal(
    $(SECTION_NAV_SEL).find("span").length,
    $(id).find(SECTION_SEL).length * 2,
    "We expect navigation have as many spans as sections exist",
  );
});
...
QUnit.test("Testing navigation `active` class when auto scrolling", function (assert) {
  ...
  assert.equal(
    $(SECTION_NAV_SEL).find(ACTIVE_SEL).closest("li").index(),
    0,
    "We expect item 1 to be active",
  );
  assert.equal(
    $(SECTION_NAV_SEL).find(ACTIVE_SEL).closest("li").length,
    1,
    "We expect a single item to be active",
  );
  ...
  for (var i = 1; i < 4; i++) {
    ...
    assert.equal(
      $(SECTION_NAV_SEL).find(ACTIVE_SEL).closest("li").index(),
      i,
      "We expect item " + (i + 1) + " to be active",
    );
    assert.equal(
      $(SECTION_NAV_SEL).find(ACTIVE_SEL).closest("li").length,
      1,
...
QUnit.test(
  "Testing navigation `active` class when auto scrolling and sliding horizontally",
  function (assert) {
...
QUnit.test("Testing navigation `active` class when scrolling & autoScrolling:false", function (
  assert,
) {
  var id = "#fullpage";
  var FP = initFullpageNew(
    id,
    Object.assign({}, allBasicOptions, { autoScrolling: false, navigation: true }),
  );
...
QUnit.test("Testing navigation `active` class when hash change by section index", function (
  assert,
) {
  var id = "#fullpage";
  var FP = initFullpageNew(id, Object.assign({}, allBasicOptions, { navigation: true }));
...
QUnit.test("Testing navigationPosition:right", function (assert) {
  ...
  var FP = initFullpageNew(
    id,
    Object.assign({}, allBasicOptions, { navigation: true, navigationPosition: "right" }),
  );
...
QUnit.test("Testing navigationPosition:left", function (assert) {
  ...
  var FP = initFullpageNew(
    id,
    Object.assign({}, allBasicOptions, { navigation: true, navigationPosition: "left" }),
  );
...
QUnit.test("Testing navigationTooltips option", function (assert) {
  ...
  var FP = initFullpageNew(
    id,
    Object.assign({}, allBasicOptions, { navigation: true, navigationTooltips: tooltips }),
  );
  for (var i = 0; i < tooltips.length; i++) {
    assert.equal(
      $(SECTION_NAV_SEL).find(SECTION_NAV_TOOLTIP_SEL).eq(i).text(),
      tooltips[i],
      `We expect nav tooltip number ${i} to be ${tooltips[i]}`,
    );
  }
});
...
QUnit.test("Testing showActiveTooltip:true", function (assert) {
  ...
  setTimeout(function () {
    assert.equal(
      $(SECTION_NAV_SEL).find(SECTION_NAV_TOOLTIP_SEL).first().css("opacity"),
      "1",
      "We expect the 1st tooltip to be visible",
    );
    done();
  }, 300);
});
...
QUnit.test("Testing click on navigation bullets span with anchors", function (assert) {
  ...
  assert.deepEqual(
    getTransform(FP.test.translate3d),
    ["0", "0", "0"],
    "We expect sections transformation to be [0, 0, 0]",
  );
  ...
  setTimeout(function () {
    assert.deepEqual(
      getTransform(FP.test.translate3d),
      ["0", `${-(windowHeight * 1)}`, "0"],
      `We expect sections transformation to be [0, ${-(windowHeight * 1)}, 0]`,
    );
