...
(function formDataModule(global, definition) {
  ...
  typeof global !== "undefined"
    ...
    ? self
    : this,
  function formDataPartialPolyfill(global) {
    // partial polyfill
    "use strict";
    ...
    function get(key) {
      var values = getAll.call(this, key);
    ...
    function FormData(form) {
      ...
      if (!form) {
        // nothing to parse, we're done here
        return;
      }
...
if (!("placeholder" in document.createElement("input")) && window.HTMLTextAreaElement) {
  ...
  window.addEventListener("DOMContentLoaded", function () {
    document
      .querySelectorAll("textarea[placeholder], input[placeholder]")
    ...
    document.body.addEventListener("DOMNodeInserted", function (event) {
      ...
      if (
        /^textarea|input$/i.test(target.tagName) &&
        target.matches("[placeholder]") &&
        (!target.data || !target.data.placeholder)
      ) {
