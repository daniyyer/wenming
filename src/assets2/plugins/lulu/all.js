  ...
  function () {
    ...
    var tips = function (arrEles) {
      ...
      arrEles.forEach(function (ele) {
        ...
        div.setAttribute(
          "style",
          "top:" + (bounding.top + scrollTop) + "px;left:" + (bounding.left + scrollLeft) + "px;",
        );
    ...
    var classList = {
      ...
      has: function (ele) {
        ...
        return (
          ele.className.split(/\s+/).filter(function (cl) {
            if (cl == className) {
              return cl;
            }
          }).length > 0
    ...
    doc.addEventListener("keydown", function (event) {
      ...
      if (keyName == "esc" && eleEscAll.length) {
        [].slice.call(eleEscAll).forEach(function (eleEsc) {
          ...
          if (
            eleFirstMatchAttrTarget &&
            eleEsc.style.display !== "none" &&
            eleEsc.clientHeight > 0
          ) {
      ...
      // ESC退出
      if (keyName == "esc") {
        eleFirstMatchAttrTarget = doc.querySelector(
          'a[data-target="' + attrTarget + '"],input[data-target="' + attrTarget + '"]',
        );
        ...
        if (attrFocus) {
          ...
          eleFirstMatchAttrTarget &&
          /ESC/.test(eleFirstMatchAttrTarget.className) == false
        ) {
          eleFirstMatchAttrTarget.click();
        }
      ...
      // 如果都符合，同时有目标子元素
      var arrEleFocusable = [].slice.call(
        target.querySelectorAll("a[href], button:not(:disabled), input:not(:disabled)"),
      );
      var index = -1;
  ...
  function () {
    ...
    **/
    var Follow = function (eleTrigger, eleTarget, options) {
      ...
      if (
        eleOffsetParent == document.body &&
        window.getComputedStyle(eleOffsetParent).position == "static"
      ) {
        numOffsetTop = 0;
        numOffsetLeft = 0;
      }
        ...
        return (
          {
            ...
            right:
              objBoundTrigger.right + objBoundTarget.width + objOffsets.x >
              numWinWidth + numScrollLeft,
            bottom:
              objBoundTrigger.bottom + objBoundTarget.height + objOffsets.y >
              numWinHeight + numScrollTop,
          }[direction] || false
        );
      };
    ...
    Follow.prototype.zIndex = function () {
      ...
      document.body.childNodes.forEach(function (eleChild) {
        ...
        if (numZIndexChild && eleTarget != eleChild && objStyleChild.display != "none") {
          ...
          numZIndexNew = Math.max(numZIndexChild + 11, numZIndexNew); //兼容coreui的modal 当modal不在body下时，zindex为modal-backdrop的zindex
        }
      });
        ...
        if (objParams.history == true) {
          ...
          if (history.replaceState) {
            history.replaceState(
              null,
              document.title,
              location.href.split("?")[0] + "?" + objURLParams.toString() + strHash,
            );
          } else {
...
(function (global, factory) {
  ...
  function (require) {
    ...
    Drop.prototype.list = function (eleTrigger, data, options) {
              ...
              if (objData[DISABLED] != true) {
                ...
                if (objData.data) {
                  strHtmlStep +=
                    ...
                    CL.add("x") +
                    '" role="listbox">' +
                    funStep(objData.data, arrCurrentIndex) +
                    "</div></div>";
                }
              } else {
                strHtmlStep +=
      ...
      eleTarget.addEventListener(
        ...
        function (event) {
          ...
          // 不是鼠标右击事件，也不是委托模式更新
          if (
            objParams.eventType != "contextmenu" &&
            objParams.selector == "" &&
            !objItemData.href
          ) {
      ...
      eleTarget.addEventListener("mouseover", function (event) {
        ...
        var eleItemSublist = eleHovered.parentElement.querySelector(
          "." + SELECTED + "[data-sublist]",
        );
        ...
        if (
          eleHovered.classList.contains(SELECTED) == false &&
          typeof eleHovered.getAttribute("data-sublist") == "string"
        ) {
          eleHovered.click();
        }
      });
    ...
    Drop.prototype.panel = function (eleTrigger, options) {
      ...
      // 按钮
      objParams.buttons.forEach(
        function (objBtn, numIndex) {
          // 避免btn为null等值报错
          objBtn = objBtn || {};
          // 按钮的默认参数
          ...
          // 按钮类名
          eleBtn.className =
            String(CL).replace("dropanel", "button") +
            " " +
            CL.add("button") +
            " " +
            (objBtn.className || "");
  ...
  function (require) {
    ...
    Object.defineProperty(Tips.prototype, "content", {
      ...
      get: function () {
        ...
        var strTitle =
          eleTrigger.getAttribute("title") || eleTrigger.getAttribute("data-title") || "";
      ...
      if (objParams.eventType == "hover") {
        ...
        document.addEventListener(
          ...
          function (event) {
            ...
            if (
              this.display == true &&
              eleTrigger.contains(eleTarget) == false &&
              this.element.tips.contains(eleTarget) == false
            ) {
              this.hide();
            }
          }.bind(this),
    ...
    Tips.prototype.show = function () {
      ...
      if (strAlign == "auto") {
        strAlign =
          eleTrigger.getAttribute("data-align") ||
          eleTrigger.getAttribute("data-position") ||
          "center";
      }
      ...
      if (
        strPositionTips &&
        strPositionTips.split("-").reduce(function (accumulator, currentValue) {
          return accumulator * 1 + currentValue * 1;
        }) == 5
      ) {
        var numHalfOffsetWidth =
          eleTrigger.getBoundingClientRect().width / 2 - eleArrow.getBoundingClientRect().width / 2;
        ...
        if (strPositionTips == "1-4" || strPositionTips == "4-1") {
          ...
          eleArrow.style.left =
            "calc(100% - " + (eleArrow.getBoundingClientRect().width + numHalfOffsetWidth) + "px)";
        }
      }
...
(function (global, factory) {
  ...
  function (require) {
    ...
    ErrorTip.prototype.show = function () {
      ...
      // aria无障碍访问增强
      eleTrigger.setAttribute(
        "aria-label",
        "错误提示：" + eleContent.textContent || eleContent.innerText,
      );
      // 两个重要标志类名
  ...
  function () {
    ...
    Loading.prototype.show = function () {
      ...
      if (objParams.observer == "attributes") {
        ...
        eleTarget.innerHTML =
          "<" + CL + ' size="' + objParams.size + ' align="center"></' + CL + ">";
        // 更新标志量
        this.loading = true;
        this.display = true;
      }
...
(function (global, factory) {
  ...
  function () {
    ...
    var Dialog = function (options) {
      ...
      if (objParams.className) {
        eleContainer.classList.add(objParams.className);
      }
    ...
    Dialog.prototype.alert = function (content, options) {
      ...
      objElement.body.innerHTML =
        '<div class="' +
        CL.add(objParams.type) +
        " " +
        CL.add("alert") +
        '">' +
        strContent +
...
(function (global, factory) {
  ...
  function (require) {
    ...
    Datalist.prototype.value = function (value) {
      ...
      // 事件
      if (
        JSON.stringify(objData) != JSON.stringify(eleInput.oldValue) ||
        this.params.index != eleInput.lastIndex
      ) {
        // 赋值时候触发的回调事件们
      ...
      eleTrigger.addEventListener(
        ...
        function (event) {
          ...
          if (
            ...
            eleTrigger.value.trim() == "" &&
            !eleTrigger.lastValue &&
            eleTrigger.getAttribute("placeholder")
          ) {
    ...
    Datalist.prototype.select = function () {
      // 选择回调
      this.callback.select.call(
        this,
        this.data[this.params.index],
          ...
          document.addEventListener("click", function (event) {
            ...
            if (eleButton.contains(target)) {
              ...
              if (eleCombobox.classList.contains(ACTIVE)) {
                // 边界判断
                var isOverflow =
                  eleDatalist.getBoundingClientRect().bottom + window.pageYOffset >
          ...
          function (event) {
            ...
            for (var indexList = 0; indexList < eleListAll.length; indexList++) {
              ...
              if (clientY >= beginY && clientY <= endY) {
                if (
                  eleList.classList.contains(SELECTED) == false &&
                  eleList.classList.contains(DISABLED) == false
                ) {
      ...
      if (window.MutationObserver) {
        ...
        var funMutationObserver = function (target, action, event) {
          ...
          if (target.tagName == "SELECT") {
            ...
            (arrMutationsList.length == 0 ||
              arrMutationsList[arrMutationsList.length - 1].node.contains(target) == false)
          ) {
            arrMutationsList.push({
              action: "refresh",
  ...
  function (require) {
      ...
      document.addEventListener(
        ...
        function (event) {
          ...
          if (
            eleTarget &&
            eleTrigger.contains(eleTarget) == false &&
            eleContainer.contains(eleTarget) == false
          ) {
            this.hide();
          }
        }.bind(this),
    ...
    Object.defineProperty(DateTime.prototype, "getCalendarData", {
      value: function (arrDate) {
        ...
        var strHtmlData =
          'data-year="' + arrDate[0] + '" data-month="' + (objNewDate.getMonth() + 1) + '"';
                ...
                if (strType == "date") {
                  ...
                  if (tr == 0 && td < numDayFirst) {
                    ...
                    if (numDayNow <= arrMonthDay[objNewDate.getMonth()]) {
                      ...
                      // 更新strHtmlFullData
                      strHtmlFullData =
                        strHtmlYearMonthData +
                        new Date(numYearNow, numMonthNow, numDayNow).toArray().join("-");
                      // 日期字符拼接
                      strHtmlDate =
                        strHtmlDate +
                        '<span class="' +
    ...
    DateTime.prototype.date = function () {
      ...
      if (
        new Date(numPrevYearGet, numPrevMonthGet, objCalender.monthDay[numPrevMonthGet]) >=
          objCalender.min &&
        datePrevMonth <= objCalender.max
      ) {
        strHtml =
          strHtml +
          '<a href="javascript:" class="' +
      ...
      // 因此判断应该从月初
      if (
        objDateNextMonth >= objCalender.min &&
        new Date(numNextYearGet, numNextMonthGet, 1) <= objCalender.max
      ) {
        strHtml =
          strHtml +
          '<a href="javascript:" class="' +
      ...
      strHtml =
        ...
        '<a href="javascript:" class="' +
        CL.date("switch") +
        '" data-type="month" role="button" aria-label="快速切换月份">' +
        arrDate.slice(0, 2).join("-") +
        "</a>\
        </div>";
      ...
      if (new Date() >= objCalender.min && new Date() <= objCalender.max) {
        ...
        strHtml =
          strHtml + '<span class="' + CL.date("item") + " " + CL.date("now") + '">今天</span>';
      }
    ...
    DateTime.prototype["date-range"] = function () {
      ...
      strHtml =
        ...
        CL.date("switch") +
        '">' +
        new Date(arrDate[0], numPrevMonth, arrDate[2]).toArray().slice(0, 2).join("-") +
        '</span>\
        </div>\
        <div class="' +
        CL.date("half") +
      ...
      strHtml =
        ...
        CL.date("switch") +
        '">' +
        objDateNextMonth.toArray().slice(0, 2).join("-") +
        "</span>\
        </div>";
      ...
      strHtml =
        ...
        CL.date("half") +
        '">' +
        this.getCalendarData(objDateNextMonth.toArray()).html +
        "</div>" +
        // 主体标签闭合
        "</div>";
    ...
    DateTime.prototype.month = function () {
      ...
      strHtml =
        ...
        '<a href="javascript:" class="' +
        CL.date("switch") +
        '" data-type="year" role="button" aria-label="快速切换年份">' +
        numYear +
        "</a>\
        </div>";
      // 3. 月份切换主体列表
      ...
      if (objThisYearMonth >= numMin && objThisYearMonth <= numMax) {
        ...
        strHtml =
          strHtml + '<span class="' + CL.date("item") + " " + CL.date("now") + '">今月</span>';
      }
    ...
    DateTime.prototype["month-range"] = function () {
      ...
      strHtml =
        ...
        CL.date("half") +
        '">' +
        this.getMonthData([numNextYear, arrDate[1], arrDate[2]]).html +
        "</div>" +
        // 主体标签闭合
        "</div>";
    ...
    DateTime.prototype.year = function () {
      ...
      if (numThisYear >= numMin && numThisYear <= numMax) {
        ...
        strHtml =
          strHtml + '<span class="' + CL.date("item") + " " + CL.date("now") + '">今年</span>';
      }
        ...
        (function () {
          ...
          for (var indexHour = 0; indexHour < 24; indexHour += numStep) {
            ...
            if (strHour >= numMin && strHour <= numMax) {
              ...
              strHtmlTime =
                strHtmlTime + '<span class="' + strClass + '">' + strHour + ":00</span>";
            }
          }
...
(function (global, factory) {
  ...
  function (require) {
    ...
    Color.prototype.events = function () {
      ...
      // 滑块的键盘支持
      eleFill.parentElement.querySelectorAll("a").forEach(
        function (eleButton) {
          eleButton.addEventListener(
            "keydown",
            function (event) {
              ...
              if (event.keyCode == 38 || event.keyCode == 40) {
                ...
                eleArrow.style.top = numTop + "px";
                eleArrow.setAttribute(
                  "aria-label",
                  ariaLabel.replace(/\d+/, Math.round((100 * numTop) / numMaxTop)),
                );
      ...
      // 圈圈的键盘访问
      // 区域背景的键盘支持
      eleCircle.parentElement.querySelectorAll("a").forEach(
        function (eleRegion) {
          eleRegion.addEventListener(
            "keydown",
            function (event) {
              // 上下左右控制
    ...
    Color.prototype.create = function () {
      ...
      var strHtmlCurrent =
        ...
        '">\
            <i class="' +
        CL.add("current", "square") +
        ' colorCurrent"></i>\
            #<input class="' +
        CL.add("current", "input") +
        '">\
        </div>';
        ...
        (function () {
          ...
          html =
            ...
            '">\
                <a href="javascript:" class="' +
            CL.add("cover", "white") +
            '" aria-label="色域背景块" role="region"></a><div class="' +
            CL.add("circle") +
            ' colorCircle"></div>\
                <svg>\
                    <defs>\
            ...
            '">\
                    ...
                    <rect x="0" y="0" width="180" height="100" fill="url(#' +
            strIdGradient +
            ')"></rect>\
                </svg></div><div class="' +
            CL.add("more", "r") +
            ...
            ' colorFill">\
                        <a href="javascript:" class="' +
            CL.add("more", "cover") +
            '" aria-label="明度控制背景条" role="region"></a>\
                        <svg>\
                        <defs>\
                            <linearGradient x1="0" y1="0" x2="0" y2="1" id="' +
            strIdGradient2 +
            '">\
            ...
            '"></stop>\
                        ...
                        <rect x="0" y="0" width="16" height="100" fill="url(#' +
            strIdGradient2 +
            ')"></rect>\
                    </svg>\
                    </div>\
                    <a href="javascript:" class="' +
            CL.add("more", "arrow") +
            ' colorArrow" role="slider" aria-label="明度控制按钮：100%"></a>\
                </div>';
      ...
      var strHtmlFooter =
        ...
        '">\
            <button class="' +
        CL.add("button", "cancel") +
        '">取消</button><button class="' +
        CL.add("button", "ensure") +
        '">确定</button>\
        </div>';
      // append
    ...
    Color.prototype.value = function (value) {
      ...
      if (typeof strValue == "string") {
        ...
        if (
          arrFixedColor.some(function (strFixedColor) {
    ...
    Object.defineProperty(Color.prototype, "getValueByStyle", {
      value: function () {
        ...
        if (eleCircle.style.left) {
          numColorH =
            parseFloat(window.getComputedStyle(eleCircle).left) /
            eleCircle.parentElement.clientWidth;
        } else {
          numColorH = 0;
        }
        if (eleCircle.style.top) {
          numColorS =
            1 -
            parseFloat(window.getComputedStyle(eleCircle).top) /
              eleCircle.parentElement.clientHeight;
        } else {
          numColorS = 1;
        }
        if (eleArrow.style.top) {
          numColorL =
            1 -
            parseFloat(window.getComputedStyle(eleArrow).top) / eleArrow.parentElement.clientHeight;
        } else {
          numColorL = 0;
        }
    ...
    Color.prototype.match = function (value) {
      ...
      if (window.getComputedStyle(eleMore).display == "none") {
        ...
        // 所有颜色一致的高亮
        var eleColorMatch = eleContainer.querySelector(
          'a[data-color="' + strValue.toUpperCase() + '"]',
        );
        if (eleColorMatch) {
    ...
    var funAutoInitAndWatching = function () {
      ...
      document.querySelectorAll(strSelector).forEach(function (eleColorInput) {
        if (
          !(eleColorInput.data && eleColorInput.data.color) &&
          window.getComputedStyle(eleColorInput).opacity == "0"
        ) {
...
(function (global, factory) {
  ...
  function (require) {
    ...
    var Range = function (element, options) {
      ...
      // 如果元素没宽度，则使用el计算的宽度
      if (
        eleRange.getAttribute("width") != "100%" &&
        eleRange.parentElement.classList.contains(CL.add("input")) == false
      ) {
    ...
    Range.prototype.event = function () {
        ...
        function (event) {
          ...
          if (target && target !== eleThumb && !eleRange.disabled) {
            var distance =
              ...
              eleRange.offsetLeft -
              eleThumb.offsetLeft -
              parseInt(window.getComputedStyle(eleThumb).width) / 2;
            var value =
              eleRange.value * 1 +
              ((numMax - numMin) * distance) /
                parseInt(eleContainer.style.width || eleContainer.clientWidth);
            this.value(value);
          }
        }.bind(this),
      );
      ...
      // 禁用状态变化检测
      if (window.MutationObserver) {
        var observerSelect = new MutationObserver(
    ...
    Range.prototype.position = function () {
      ...
      var numMin = objNumber.min;
      // 计算百分比
      this.element.track.style.borderLeftWidth =
        (parseInt(eleContainer.style.width || eleContainer.clientWidth) * (strValue - numMin)) /
          (numMax - numMin) +
        "px";
      // aria同步
    ...
    var funAutoInitAndWatching = function () {
      ...
      document.querySelectorAll(strSelector).forEach(function (eleRange) {
        if (
          !(eleRange.data && eleRange.data.range) &&
          window.getComputedStyle(eleRange).visibility == "hidden"
        ) {
          new Range(eleRange);
        }
      });
    ...
    document.validate = (function () {
      return {
        reg: {
          ...
          number: "^\\-?\\d+(\\.\\d+)?$",
          url:
            "^(http|https)\\:\\/\\/[a-z0-9\\-\\.]+\\.[a-z]{2,3}(:[a-z0-9]*)?\\/?([a-z0-9\\-\\._\\:\\?\\,\\'\\/\\\\\\+&amp;%\\$#\\=~])*$",
          tel: "^1\\d{10}$",
          zipcode: "^\\d{6}$",
        ...
        getReportText: function (element) {
          ...
          var strName =
            ...
            (function () {
              ...
              document.querySelectorAll("label[for=" + strId + "]").forEach(function (eleLabel) {
                ...
                // 去除数字和冒号
                var strLabelCloneText = eleLabelClone.innerHTML
                  .trim()
                  .replace(/\d/g, "")
                  .replace("：", "");
                // 内容最长的那个元素作为提示文字
          ...
          if (objValidateState.valueMissing) {
            ...
            if (!strFinalText) {
              // 然后确定是否是多值
              var isMultiple =
                element.hasAttribute("multiple") && element.value.split(",").length > 1;
            ...
            if (!strFinalText) {
              ...
              if (strType == "number" || strType == "range") {
                ...
                strFinalText =
                  "请" +
                  (element.hasAttribute("readonly") ? "选择" : "输入") +
                  "有效的值。" +
                  (strName || "") +
                  "间隔是" +
                  numStep;
            ...
            if (objValidateState.tooLong) {
              ...
              if (!strFinalText) {
                var strMinLength = element.getAttribute("data-val-length-min");
        ...
        getRemoteValidation: function (element) {
          ...
          if (
            ...
            element.customValidate.remoteUrl &&
            element.customValidate.remoteQueryName &&
            element.value
          ) {
            if (element.pending === undefined) {
              remoteInValid = true;
              let submittriger = false;

              var eleForm = element.closest("form");
              // 如果isImmediated未定义，说明是第一次提交，则验证通过后再触发一次提交操作，如果有immediate定义，说明提交过，则更新状态时不触发提交
              if (eleForm && !eleForm.isImmediated) {
                submittriger = true;
              }
              // 如果是部分验证，如远程验证邮箱然后发送验证码
              if (
                element.getAttribute("data-parcial") &&
                element.getAttribute("data-pending-click")
              ) {
                submittriger = false;
              }
              //request 验证通过直接触发提交
              this.requestRemoteValidate(element, submittriger);
            } else {
              if (element.pending) {
                remoteInValid = true;
              } else {
        ...
        requestRemoteValidate: function (element, submitTrigger) {
            ...
            if (this.readyState == 4) {
              if (this.status == 200) {
                ...
                if (valid) {
                  ...
                  element.setAttribute("data-pending-click", false);
                }
              } else {
              ...
              if (submitTrigger) {
                element.customValidate.owner.element.form.dispatchEvent(
                ...
                // 如果是局部验证
                let parcialId = element.getAttribute("data-parcial-target");
                if (parcialId) {
                  let button = document.getElementById(parcialId);
                  if (element.getAttribute("data-pending-click")) {
                    element.setAttribute("data-js-click", true);
                    button.click();
                    element.setAttribute("data-pending-click", false);
                  }
                }
          ...
          for (var k = 0; k < paramNames.length; k++) {
            ...
            // }
            var eleForm = element.closest("form");
            if (eleForm.querySelector(`input[name=${paramName}]`)) {
              paraVale = eleForm.querySelector(`input[name=${paramName}]`).value;
            }
            if (eleForm.querySelector(`select[name=${paramName}]`)) {
              paraVale = eleForm.querySelector(`select[name=${paramName}]`).value;
            }
            remoteValue = remoteValue + paramName + "=" + paraVale + "&";
          }
          remoteValue = remoteValue.slice(0, -1);

          element.currentXHR.open(
            "GET",
            element.customValidate.remoteUrl + "?" + encodeURI(remoteValue),
            true,
          );
          ...
          if (element.hasAttribute("data-val-required")) {
            ...
            if (strType == "radio") {
              ...
              if (element.name && eleForm) {
                eleRadios = eleForm.querySelectorAll(
        ...
        getRangeState: function (element) {
          ...
          if (strType.slice(-6) != "-range") {
            ...
            if (
              (strType == "hour" || strType == "minute" || strType == "time") &&
              strAttrMin &&
              strAttrStep
            ) {
              ...
              if (
                ...
                (minuteValue != minuteMin ||
                  (strValue.split(":")[0] - strAttrMin.split(":")[0]) % strAttrStep != 0)
              ) {
        ...
        isOut: function (element) {
          ...
          return (
            objValidateState.badInput ||
            objValidateState.rangeOverflow ||
            objValidateState.rangeUnderflow ||
        ...
        getLengthState: function (element) {
          ...
          //  大小限制
          var strAttrMinLenght = element.getAttribute("data-val-length-min");
        ...
        styleError: function (element, valid) {
          ...
          var eleForm =
            element.closest("form") || (element.customValidate && element.customValidate.owner);
        ...
        errorTip: function (element, content) {
          ...
          var funShow = function () {
              ...
              onShow: function (eleTrigger, eleTips) {
                ...
                if (numOffsetX < 0) {
                  eleTips.style.marginLeft =
                    0.5 * (eleTips.clientWidth - eleTarget.clientWidth) + "px";
                } else {
            ...
            if (content.indexOf("内容长度") != -1 && content.indexOf("大") != -1) {
              ...
              // }
              // 获得最大长度的索引位置
              var strAttrMaxLength =
                eleControl.maxlength ||
                eleControl.getAttribute("data-val-length-max").replace(/\D/g, "");

              if (numLength && strAttrMaxLength) {
                document.validate.selectRange(
                  element,
                  document.validate.getLength(element, strAttrMaxLength),
                  numLength,
                );
              }
        ...
        getTarget: function (element) {
          ...
          if (strType == "radio" && objStyle.opacity != "1") {
            ...
            var eleTargetRel =
              document.getElementById(eleTarget.getAttribute("data-target")) || element.dataTarget;
            if (eleTargetRel) {
              eleTarget = document.validate.getTarget(eleTargetRel);
            }
          } else if (strType == "textarea" || strTag == "textarea") {
            if (
              element.classList.contains("ui-textarea") == false &&
              element.parentElement.querySelector(".ui-textarea")
            ) {
              eleTarget = element.parentElement.querySelector(".ui-textarea");
            }
          } else if (strTag == "input") {
            if (
              element.classList.contains("ui-input") == false &&
              element.parentElement.querySelector(".ui-input")
            ) {
    ...
    var Validate = function (element, callback, options) {
      ...
      eleForm.querySelectorAll("input, select, textarea").forEach(function (eleInput) {
        ...
        if (eleInput.getAttribute("data-val-remote-additionalfields")) {
          validateOption.remoteQueryName = eleInput.getAttribute(
        ...
        for (let i = 0; i < arrValidateKey.length; i++) {
          if (eleInput.getAttribute("data-report-" + arrValidateKey[i])) {
            ...
            validateOption.report[arrValidateKey[i]] = eleInput.getAttribute(
              "data-report-" + arrValidateKey[i],
            );
        ...
        if (
          (validateOption.id && validateOption.remoteUrl && validateOption.remoteQueryName) ||
          dataCusReportExist
        ) {
          let exist = false;
          if (options && options.validate) {
            for (let i = 0; i < options.validate.length; i++) {
              if (options.validate[i].id === validateOption.id) {
                ...
                options.validate[i].report = Object.assign(
                  {},
                  options.validate[i].report,
                  validateOption.report,
                );
                break;
              }
            }
      ...
      // 还原禁用的提交和关闭按钮
      eleForm
        .querySelectorAll('[type="submit"]:disabled, [type="image"]:disabled')
        .forEach(function (eleSubmit) {
          eleSubmit.disabled = false;
        });

      // 干掉默认的提交
      eleForm.addEventListener(
        "submit",
        function (event) {
      ...
      // 遍历元素并匹配
      eleForm.querySelectorAll("input, select, textarea").forEach(
        function (eleInput) {
          var strId = eleInput.id;
    ...
    Object.defineProperty(Validate.prototype, "count", {
      value: function () {
        ...
        eleForm.querySelectorAll("input, textarea").forEach(function (element) {
          ...
          // 获得限制大小
          var strAttrMinLength = element.getAttribute("data-val-length-min");
          ...
          // 有没有label元素
          var eleLabel =
            element.parentElement.querySelector("." + CL.add("count")) ||
          ...
          if (!eleLabel) {
            ...
            eleLabel.setAttribute("for", strId);
            eleLabel.innerHTML =
              "<span>0</span><slash>/</slash><span>" +
              (strAttrMinLength ? strAttrMinLength + "-" : "") +
              strAttrMaxLength +
              "</span>";
            // 插入到后面
          ...
          var funCount = function () {
            ...
            if (
              length != 0 &&
              (length > strAttrMaxLength || (strAttrMinLength && length < strAttrMinLength))
            ) {
      ...
      eleForm.querySelectorAll("input, select, textarea").forEach(
        function (element) {
          ...
          if (strType == "radio" || strType == "checkbox") {
            ...
            /select/.test(strType) ||
            /range|date|time|hour|minute|hidden/.test(strAttrType)
          ) {
            element.addEventListener(
              "change",
              function () {
...
(function (global, factory) {
  ...
  function () {
    ...
    Pagination.prototype.events = function () {
      ...
      var numTotal = this.params.total * 1 || 0;
      var numPer = this.params.per * 1 || 1;

      var numPageSize = Math.ceil(numTotal / numPer) || 1;
      // 委托点击事件
      elePagination.addEventListener(
        "click",
        function (event) {
          // 一些元素
          var eleTarget = event.target;
          ...
          // 未获取到元素返回
          if (!eleClicked) {
            if (eleTarget.getAttribute("id") === "jump") {
              let inputNum = parseInt(eleTarget.previousSibling.value);
              if (isNaN(inputNum)) {
                return;
              } else {
                if (inputNum < 0) {
                ...
                if (inputNum > numPageSize) {
                  inputNum = numPageSize;
                }
                if (inputNum == this.params.current) {
                  return;
                } else {
                  let triggerPage = elePagination.querySelector("a[data-page='" + inputNum + "']");
                  if (!triggerPage) {
                    triggerPage = document.createElement("a");
                    triggerPage.setAttribute("href", "javascript:");
                    triggerPage.setAttribute("class", "ui-page");
                    triggerPage.setAttribute("data-page", inputNum);

                    elePagination.appendChild(triggerPage);
                  }
                  triggerPage.click();
                  return;
                }
    ...
    Pagination.prototype.create = function () {
      ...
      return (
        ...
        '">' +
        strHtmlPage +
        '<input class="form-control inbk w-60 "><button class="btn  btn-link" id="jump">跳转到</button></div>'
      );
    };
  ...
  function (require) {
    ...
    Table.prototype.events = function () {
      ...
      eleTable.addEventListener(
        ...
        function (event) {
          ...
          eleAllTdCheckbox.forEach(function (eleTdCheckbox) {
            eleTdCheckbox
              .closest("tr")
              .classList[eleTdCheckbox[CHECKED] ? "add" : "remove"](SELECTED);
          });
      ...
      eleTable.addEventListener("click", function (event) {
        ...
        if (
          eleTarget &&
          /^(?:a|input|textarea|tbody|i|select|label|th)$/i.test(eleTarget.tagName) == false
        ) {
          eleCheckbox =
            eleTarget.closest("tr") &&
            eleTarget.closest("tr").querySelector("td:first-child " + CL.checkbox + ":enabled");
          if (eleCheckbox) {
            eleCheckbox.click();
          }
        }
      ...
      if (elePer) {
        ...
        // 下拉
        this.dropList = new Drop(elePer).list(
          objParams.list.map(function (number) {
            return {
              value: number,
            };
          }),
    ...
    Table.prototype.ajax = function (options) {
      ...
      var funAjax = function () {
        ...
        if (
          !history.pushState &&
          /\/\//.test(strUrlAjax) &&
          strUrlAjax.split("//")[1].split("/")[0] != location.host
        ) {
      ...
      if (window.getComputedStyle(eleLoading).display == "none") {
        ...
        eleLoading.style.height =
          eleTable.clientHeight - (eleThead ? eleThead.clientHeight : 0) + "px";
